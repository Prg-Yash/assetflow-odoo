  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-1">
        <h2 className="text-2xl font-bold text-foreground tracking-tight">Allocation &amp; Transfer</h2>
        <p className="text-sm text-muted-foreground">Allocate available assets or request transfers for already-assigned ones.</p>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card overflow-hidden">
        <div className="p-6 sm:p-8 space-y-6">
          ...
          <Field label="Asset">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-border bg-input pl-10 pr-10 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent focus:border-accent transition"
              />
              {selectedAsset && (
                <button onClick={handleClear} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground/80 hover:text-foreground transition-colors">
                  <X className="w-4 h-4" />
                </button>
              )}
              ...
              {dropdownOpen && searchResults.length > 0 && !selectedAsset && (
                <div className="absolute top-full left-0 right-0 mt-1.5 z-50 rounded-xl border border-border bg-popover shadow-2xl shadow-black/50 py-1.5 max-h-64 overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-150">
                  {searchResults.map((a) => (
                    <button
                      key={a.id}
                      onClick={() => selectAsset(a)}
                      className="w-full text-left px-4 py-3 flex items-center justify-between gap-4 hover:bg-muted transition-colors"
                    >
          ...
          {selectedAsset && isRetired && (
            <div className="flex items-start gap-3 rounded-xl border border-border bg-card px-5 py-4 animate-in fade-in slide-in-from-top-2 duration-200">
              <AlertCircle className="w-5 h-5 text-muted-foreground shrink-0 mt-0.5" />
              <div className="space-y-1">
                <p className="text-sm font-semibold text-muted-foreground">Asset is Retired</p>
                <p className="text-xs text-muted-foreground/80">Retired assets cannot be allocated or transferred.</p>
              </div>
            </div>
          )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-300">
                  ...
                  {isAllocated && (
                    <Field label="From">
                      <div className="flex items-center gap-2.5 rounded-lg border border-border bg-card px-3 py-2.5">
                        ...
                        <span className="text-sm text-foreground/70">{selectedAsset.assignedTo}</span>
                      </div>
                    </Field>
                  )}
              ...
              <Field label="Reason">
                <textarea
                  ...
                  className="w-full rounded-lg border border-border bg-input px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent focus:border-accent transition resize-none"
                />
              </Field>
                  ...
                  {history.map((entry, i) => (
                    <div key={entry.id} className="relative flex gap-4 items-start group">
                      ...
                      <div className={`absolute -left-6 top-1 w-[11px] h-[11px] rounded-full border-2 shrink-0 ${
                        i === 0 ? 'border-accent bg-accent/30' : 'border-border bg-card'
                      }`} />

                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 flex-wrap">
                          <span className="text-xs text-foreground/35 font-medium tabular-nums shrink-0 w-24">{entry.date}</span>
          ...
          <div className="border-t border-border px-6 py-16 text-center">
            <div className="space-y-3">
              <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-card border border-border mx-auto">
                <Package className="w-7 h-7 text-foreground/15" />
              </div>
              <div>
                <p className="text-sm text-muted-foreground/80 font-medium">Search for an asset to get started</p>
                <p className="text-xs text-foreground/20 mt-1">Look up by asset tag (e.g. AF-0114) or name</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-foreground/80">Recent Requests</h3>
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted">
                  <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Asset</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Type</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">From → To</th>
                ...
                {[
                  ...
                  <tr key={i} className="hover:bg-muted transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-muted-foreground text-sm">
                      {row.from} <ArrowRight className="w-3 h-3 inline mx-1 text-muted-foreground/70" /> {row.to}
                    </td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5 text-muted-foreground text-xs tabular-nums">{row.date}</td>
                  </tr>
                ))}
              </tbody>
            </table>
